...
public class Kpi2_1WorksheetHandler : WorksheetExportHandlerBase
{
    private SprintReportEntity _sprintReportEntity;

    public Kpi2_1WorksheetHandler(ExcelPackage excelPackage, string listName, SprintReportEntity sprintReportEntity)
    {
        ListName = listName;
    ...
    private void FillReworksByProjects(KeyValuePair<string, List<IssueEntity>> projectIssues)
    {
        ...
        var allClosedIssues = projectIssues.Value.Where(i => i.Status == JiraConstants.Closed)
            .Where(i => i.Type == JiraConstants.Bug);
    ...
    private void FillHeaders()
    {
        ...
        CurrentWorksheet.SetValue(headerRow, AverageResolvingTimeForIssues, "Среднее время решения Ошибок в часах");
